...
enum DomainError:
  ...
  def message: String =
    this match
      case EmptyPresentationTitle    => "Presentation title must not be blank"
      case PresentationWithoutSlides =>
      ...
      case EmptySlideTitle             => "Slide title must not be blank"
      case SlideWithoutElements(title) =>
        s"Slide '$title' must contain at least one element"
      case EmptyParagraph  => "Paragraph text must not be blank"
      case EmptyBulletList => "Bullet list must contain at least one item"
      case EmptyBulletItem(index) =>
        s"Bullet item at index $index must not be blank"
      case EmptyCodeLanguage        => "Code block language must not be blank"
      case EmptyCodeBlock           => "Code block source must not be blank"
